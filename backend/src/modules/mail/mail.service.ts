...
export class MailService {
    ...
    constructor() {
        ...
        const fromEmail = process.env.SMTP_FROM || 'noreply@loop.local';
        const fromName = process.env.APP_NAME || 'LOOP';
        this.sentFrom = new Sender(fromEmail, fromName);

        if (apiKey) {
            this.mailerSend = new MailerSend({ apiKey });
            this.logger.log(`✅ Mail service configured (MailerSend SDK) — from: ${fromEmail}`);
        } else {
            this.logger.error('❌ Mail service NOT configured — set MAILERSEND_API_KEY in .env');
        }
    }
    ...
    async sendMail(to: string, subject: string, html: string, text?: string): Promise<boolean> {
        ...
        try {
            ...
            this.logger.error(`❌ Failed to send email to ${to}: ${err.message || JSON.stringify(err)}`);
            return false;
        }
    }
